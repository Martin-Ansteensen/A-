...
class Node:
    ...
    def __init__(self, x, y, parent):
        ...
        self.cost = self.start_cost*0.4 + self.end_cost 
...
def find_path():
    ...
    while current_node_xy != end: # Run until the current node is the end node
        ...
        for node in all_nodes:
            #print("The node costs: " + str(node.cost) + " and has these coordinates: " + str(node.x) + " " + str(node.y))
    ...
    if solution_possible:
        ...
        #     draw_cell_v2(solution[-1].y, solution[-1].x,  (0,0,255)) 
        #     if solution[-1].x_from_start == 0 and solution[-1].y_from_start == 0:
        #         print(solution[-1].x_from_start)
        #         print(solution[-1].y_from_start)
        #         break
        #     #wait()
